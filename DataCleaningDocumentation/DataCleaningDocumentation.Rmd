...
knitr::opts_knit$set(root.dir = "C:/Users/Priya/Desktop/D2D_Data_Cleaning") 
```

## Load data

```{r loadData, echo=FALSE}
data <- read.csv("site_phenometrics_data.csv")
```
...
for(i in 1:nrow(data)){
  if(data$First_Yes_Sample_Size[i] == 0 & data$Last_Yes_Sample_Size[i] > 0) {
    ...
    data$Mean_First_Yes_DOY[i] <- data$Mean_Last_Yes_DOY[i]
    data$NumLastNo[i] <- 0
  }
}
```
...
for(i in 1:nrow(data)){
  if(data$Last_Yes_Sample_Size[i] == 0 & data$First_Yes_Sample_Size[i] > 0) {
    ...
    data$Mean_Last_Yes_DOY[i] <- data$Mean_First_Yes_DOY[i]
    data$NumFirstNo[i] <- 0
  }
}
```
...
The columns are replaced to get it convenient for reading
```{r rename column}
data <- data %>% rename(SiteID = Site_ID)
data <- data %>% rename(SiteName = Site_Name)
data <- data %>% rename(SpeciesID = Species_ID)
data <- data %>% rename(SpeciesDesc = Common_Name)
data <- data %>% rename(PhenophaseID = Phenophase_ID)
data <- data %>% rename(PhenophaseCategory = Phenophase_Category)
data <- data %>% rename(PhenophaseDesc = Phenophase_Description)
data <- data %>% rename(Year = Mean_First_Yes_Year)
data <- data %>% rename(FirstYes = Mean_First_Yes_DOY)
data <- data %>% rename(LastYes = Mean_Last_Yes_DOY)
data <- data %>% rename(NumFirstNo = Mean_NumDays_Until_Next_No)
data <- data %>% rename(NumLastNo = Mean_NumDays_Since_Prior_No)
```
...
**_Proofreader statement_** : I, have reviewed and proofread the following work and agree that it is ready for submission.
