...
for(i in 1:nrow(data)){
  if(data$First_Yes_Sample_Size[i] == 0 & data$Last_Yes_Sample_Size[i] > 0) {
    ...
    data$irst_Yes_Year[i] <- data$Last_Yes_Year[i]
...
```{r selectrow}
cols <- c('SiteID', 'SiteName', 'Latitude', 'Longitude', 'SpeciesID', 'SpeciesType',
          'SpeciesDesc', 'Kingdom', 'PhenophaseID', 'PhenophaseDesc',
          'First_Yes_Year', 'Mean_First_Yes_DOY', 'Last_Yes_Year', 'Mean_Last_Yes_DOY',
          'NumLastNo', 'NumFirstNo')
data <- data[cols]
...
```{r selectcol}
data['LastNo'] <- data$Mean_First_Yes_DOY - data$NumLastNo
data['FirstNo'] <- data$Mean_Last_Yes_DOY + data$NumFirstNo
```  

The temporal integrity of the following dates was checked, and any rows that did not follow this rule were de-selected.

```{r deselect rows}
data %>% filterwhich(!(data$LastNo <= data$Mean_First_Yes_DOY &
          data$Mean_First_Yes_DOY <= data$Mean_Last_Yes_DOY &
          data$Mean_Last_Yes_DOY <= data$FirstNo))
```
